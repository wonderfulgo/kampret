...
Description=Xray Service By SSH
...
EOF
sed -i '$ ilocation = /xrayvless' /etc/nginx/conf.d/xray.conf
...
sed -i '$ ilocation = /xrayvmess' /etc/nginx/conf.d/xray.conf
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
...
sleep 2
bash acme.sh --register-account -m jonada.ada@gmail.com
